...
const visibleYear = computed<number>(
  () => calendarPlaceholder.value?.year ?? t.year,
);
const visibleMonth = computed<number>(
  () => calendarPlaceholder.value?.month ?? t.month,
);

// ── Queries ───────────────────────────────────────────────
const { activityDates, loading: activityLoading } = useWorkoutActivityDates(
  visibleYear,
  visibleMonth,
);
...
const { sessions: daySessions, loading: dayLoading } = useWorkoutSessions(
  100,
  0,
  null,
  selectedDateKey,
...
const { session: detailSession, loading: detailLoading } =
  useWorkoutSession(selectedSessionId);
  ...
  <div class="pb-10">
    ...
    <div class="flex flex-col lg:flex-row gap-6 lg:gap-8">
      ...
      <div class="lg:w-72 lg:shrink-0 space-y-3">
        <div class="flex flex-col gap-4 border rounded-lg bg-card">
          <Calendar
        ...
        <div
          ...
          class="h-0.5 rounded-full bg-muted overflow-hidden"
        >
          <div
            class="h-full w-2/5 bg-primary/50 rounded-full animate-pulse mx-auto"
          />
        </div>
      </div>
        ...
        <div class="flex items-baseline justify-between gap-2 min-h-[1.5rem]">
          ...
          >
            <span class="flex items-center gap-1"
              ><Layers class="w-3.5 h-3.5" />{{ dayStats.exercises }}</span
            >
            <span class="flex items-center gap-1"
              ><Flame class="w-3.5 h-3.5" />{{ dayStats.sets }} sets</span
            >
            <span v-if="dayStats.durationMs > 0" class="flex items-center gap-1"
              ><Clock class="w-3.5 h-3.5" />{{
                formatMs(dayStats.durationMs)
              }}</span
            >
          </div>
        </div>
                  ...
                  <div class="min-w-0">
                    <p class="font-semibold text-base leading-tight truncate">
                      {{
                        ...
                        $t("workoutHistory.unnamedWorkout")
                      }}
                    </p>
                    <p
                      class="text-xs text-muted-foreground mt-0.5 flex items-center gap-1.5"
                    >
      ...
      <Sheet :open="drawerOpen" @update:open="(v: boolean) => (drawerOpen = v)">
        ...
        <Dialog v-model:open="deleteDialogOpen">
          <DialogContent class="max-w-sm">
            <DialogHeader>
              <DialogTitle>{{
                $t("workoutHistory.deleteConfirmTitle")
              }}</DialogTitle>
              <DialogDescription>{{
                $t("workoutHistory.deleteConfirmDescription")
              }}</DialogDescription>
            </DialogHeader>
          ...
          <div class="flex-1 overflow-y-auto px-6 py-5 min-h-0">
            ...
            >
              ...
              <p class="text-muted-foreground text-sm">
                {{ $t("workoutHistory.noSetsCompleted") }}
              </p>
            </div>
                ...
                >
                  ...
                  <p class="text-sm font-semibold mb-2">
                    {{
                      ...
                      $t("workoutHistory.unknownExercise")
                    }}
                  </p>
                    ...
                    <table class="w-full">
                      <thead>
                        <tr class="bg-muted/50 text-xs text-muted-foreground">
                          ...
                          <th
                            v-for="val in exercise.sets.find(
                              (s) => s.completed_at,
                            )?.values ?? []"
                      ...
                      <tbody class="divide-y divide-border">
                        <tr
                          v-for="(set, i) in exercise.sets.filter(
                            (s) => s.completed_at,
                          )"
                          :key="set.id"
                        ...
                        >
                          <td
                            class="px-3 py-2 text-muted-foreground font-medium"
                          >
                            {{ i + 1 }}
                          </td>
                          <td
